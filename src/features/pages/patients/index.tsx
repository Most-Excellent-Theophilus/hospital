...
import { usePatientsInfinite } from "./infinite.scroll";
...
const PatientsModule = () => {
  ...
  const flatRows = data?.pages.flatMap((p) => p.data) ?? [];
      ...
      >
        <div className="flex justify-between space-x-3.5">
          <div className="flex-1 flex items-center">
            <div className="pr-9" ><h1 className="text-3xl text-primary font-bold underline ">{data?.pages.at(-1)?.count || 0} Patients </h1></div>
            <DataTablePagination count={Math.floor(Number(data?.pages.at(-1)?.count || 1) / pageSize)} table={table} />
          </div>
