...
Heading.configure({
  ...
  HTMLAttributes: {
    ...
    class: "scroll-mt-20", // optional for anchor linking
  },
}),
    Typography,
  ...
  return (
    <TooltipProvider >
      <div className={cn(className, "[&_.ProseMirror-focused]:outline-none w-full")}>
...
export const EditorFloatingMenu = ({
  ...
  return (
    <FloatingMenu
      className={cn("flex items-center place-self-end  bg-secondary", className)}
      editor={editor ?? null}
      {...props}
    />
  );
      ...
      <PopoverContent align="start" className="w-60 p-0" sideOffset={10}>
        <form className="flex p-1" onSubmit={handleSubmit}>
          ...
          {editor.getAttributes("link").href ? (
            ...
            <Button  type='button'  className="h-8" size="icon" variant="secondary">
              <CheckIcon size={12} />
            </Button>
          )}
        </form>
